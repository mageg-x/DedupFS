...
// DoRestore 执行实际的数据恢复操作
func DoRestore(dataDir, toPath string) error {
...
func buildNodeTree(inodeMap map[uint64]*dfs.BackUpINode) (*dfs.Tree, error) {
	...
	for len(stack) > 0 {
		...
		// 插入到树中
		if current != 1 {
			if _, err := tree.Insert(parent, current, inodeMap[current].Name); err != nil {
	...
	if len(orphanNodes) > 0 {
		...
		for _, node := range orphanNodes {
			if _, err := tree.Insert(1, node, inodeMap[node].Name); err != nil {
