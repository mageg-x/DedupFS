...
func (m *inodeModel) loadChunkHex() {
	...
	if len(chunk.Data) == 0 {
		...
		if chunkBytes, err := ipccmd.InvokeXattr(m.mountPoint, "/", attrName); err == nil && len(chunkBytes) > 0 {
...
func RenderInode(mountPoint string, inode *dfs.INode, infoRows, inodeRows [][]string, chunkLines []string) error {
	// 初始化模型
	model := inodeModel{
		infoRows:           infoRows,
		inodeRows:          inodeRows,
