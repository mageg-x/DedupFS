...
func startServerAction(cmd *cobra.Command, args []string) error {
	...
	socketDir := filepath.Dir(ipcpath)
	...
	if ipc.IsServerRunning(ipcpath) {
		logger.Errorf("server is already running at %s", ipcpath)
	...
	// 确保在函数退出时停止 IPC 服务器
	defer func() {
		logger.Debugf("stopping ipc server on exit")
		mount.CleanupMounts()
	}()
...
func stopServerAction(cmd *cobra.Command, args []string) error {
	ipcpath := ipc.GetPath("xxxx")
	// Check if server is running
	if !ipc.IsServerRunning(ipcpath) {
		logger.Errorf("server is not running at %s", ipcpath)
		return nil
	}

	logger.Infof("sending stop command to server at %s", ipcpath)

	if err := ipccmd.InvokeStopSvr("xxxx"); err != nil {
		logger.Errorf("failed to send stop command to %s: %v", ipcpath, err)
		return nil
	}
...
func mountAction(_ *cobra.Command, args []string) error {
	...
	if err := ipccmd.InvokeMount(mountPoint, dataDir, chunkConf, blockConf); err != nil {
...
func startIpcServer(ctx context.Context, mountPoint string) error {
	logger.Debugf("starting ipc server for mount point %s", mountPoint)
	// 创建 IPC 服务器
	ipcpath := ipc.GetPath(mountPoint)
	server := ipc.NewServer(ipcpath)
	defer server.Close()

	server.Register("mount", ipccmd.HandleMountCommand)
	...
	server.Register("stop", func(ctx context.Context, req *ipc.Request) *ipc.Response {
	...
	logger.Debugf("starting ipc server")
	server.Start(ctx)
	logger.Debugf("ipc server stopped")
...
// Cmd 启动命令行模式
func runCmd() {
	logger.Debugf("running in command line mode")
