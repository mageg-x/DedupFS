...
var debugCmd = &cobra.Command{
	Use:     "debug [mountpoint] block <blockID>",
	Short:   "Interactive debug UI for block inspection",
	Args:    cobra.ExactArgs(3),
	RunE:    debugAction,
	Example: "dedupfs debug /data/dfs block abc123def456",
}

func debugAction(cmd *cobra.Command, args []string) error {
	mountPoint := args[0]
	blockID := args[2]
	return debugBlockAction(mountPoint, blockID)
}

type focus int

const (
	focusChunk focus = iota
	focusHex
)
...
type model struct {
	...
	viewport viewport.Model // now scrollable
...
func (m *model) scrollToLine(vp *viewport.Model, lineIndex, totalLines int) {
...
func (m *model) scrollToSelectedChunk() {
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case msg.String() == "tab":
			if m.focus == focusChunk {
				m.focus = focusHex
			} else {
				m.focus = focusChunk
			}

		case m.focus == focusChunk:
			switch msg.String() {
			case "up":
...
func highlightLine(lines []string, lineIndex int, width int) string {
...
func (m model) View() string {
	...
	if len(m.hexLines) > 0 {
		...
		hexContentWithHighlight = highlightLine(m.hexLines, currentLine, m.hexVP.Width)
	}
	m.hexVP.SetContent(hexContentWithHighlight)

	// Borders
	chunkBorderStyle := lipgloss.NormalBorder()
	hexBorderStyle := lipgloss.NormalBorder()

	if m.focus == focusChunk {
		chunkBorderStyle = lipgloss.ThickBorder()
	} else {
		hexBorderStyle = lipgloss.ThickBorder()
	}
...
func debugBlockAction(mountPoint, blockID string) error {
	...
	if err != nil {
		...
		for i, chunk := range block.Header.ChunkList {
			idx := i + 1
			line := fmt.Sprintf("%2d. %-40s  %12s bytes", idx, chunk.Hash, formatSize(int64(chunk.Size)))
